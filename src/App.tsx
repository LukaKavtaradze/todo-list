...
//   function createTodo() {
//     if (inputValue.trim() !== "") {
//       // Define an object that adheres to the Todos interface
//       const newTodo: Todos = {
//         title: inputValue,
//         id: todo.length + 1,
//         checked: false
...
//       // Create a new array by spreading the existing todo array and adding the newTodo
//       const updatedTodos: Todos[] = [...todo, newTodo];

//       // Set the updatedTodos state
...
function App() {
  const [isDark, setIsDark] = useState<boolean>(true);
  const [inputValue, setInputValue] = useState<string>("");
  const [todo, setTodo] = useState<any[]>([]); // Changed type to any[] as it's initially an empty array
  const [filter, setFilter] = useState<string>("all");

  function getTodos(event: React.ChangeEvent<HTMLInputElement>) {
  ...
  function createTodo() {
    if (inputValue.trim() !== "") {
      ...
      const updatedTodos: Todo[] = [...todo, newTodo];
      setTodo(updatedTodos);
  ...
  function deleteCompleted() {
    const deleted = todo.filter((item) => !item.checked); // Changed condition to !item.checked
    setTodo(deleted);
  ...
  function deleteTodo(index: number) {
    const newTodos = [...todo.slice(0, index), ...todo.slice(index + 1)]; // Changed to use slice to remove item at index
    setTodo(newTodos);
  }
      ...
      <div id="heading" className="w-full">
        <div
          className={`bg-desktop-${
            isDark ? "dark" : "light"
          } h-52 bg-no-repeat bg-cover bg-center md:h-80`}
        >
          <header className="flex flex-row justify-between md:items-center w-[327px] md:w-[541px] mx-auto pt-10 md:pt-20 px-0">
            <h1 className="text-white text-2xl font-bold tracking-[10px] uppercase cursor-pointer md:text-4xl md:tracking-[15px]">
              todo
                ...
                <div className="relative flex items-center gap-3 md:gap-5 flex-row ">
                  ...
                  <li>
                    <p
                      ...
                      }  break-words max-w-[200px] md:max-w-[400px]`}
                    >
                      {item.title}
                    </p>
                  </li>
              ...
              >
                ...
                >
                  <span
                    onClick={() => setFilter("all")}
                    className={` font-bold ${
                      isDark ? "hover:text-white" : "hover:text-[#494c6b]"
                    } cursor-pointer ${
                      filter === "all" ? "text-[#3A7CFD]" : "text-[#575a78]"
                    }`}
                  >
                    All
                  </span>
                  <span
                    ...
                    className={` font-bold ${
                      ...
                      filter === "active" ? "text-[#3A7CFD]" : "text-[#575a78]"
                    }`}
                  >
                    Active
                  </span>
                ...
                <h3 onClick={deleteCompleted} className={`${
              ...
              >
                ...
                <span
                  ...
                  className={` font-bold ${
                    ...
                    filter === "completed" ? "text-[#3A7CFD]" : "text-[#575a78]"
                  }`}
                >
                  Completed
                </span>
